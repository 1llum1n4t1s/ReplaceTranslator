...
- **段落内フォント統一（OCR 行ごとのサイズばらつき対策）**: OCR(特にローカル DBNet)は行ごとに別 box を返すため、行ごとに `fitCanvasFont` するとサイズがバラつく。`computeGroupFonts` が `shouldLinkRows` で連結した flat 同色段落ごとに「全行が収まる**最小**フォント」を求め、`drawInpaintBlock`/`drawTextOnly` の `forcedFont` に渡す（収まらない box は個別 fit に落としてはみ出し防止）。`renderInpaint`/`renderInpaintNeural` 両経路で適用
