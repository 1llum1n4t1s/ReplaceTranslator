...
- **混在言語のピンポイント翻訳**: `providers.js` の `buildSystemPrompt` で「すでに target 言語の要素はそのまま、それ以外だけ翻訳し同数同順 JSON で返す」と LLM に指示。クライアント側は accept() で数値/記号/URL/1文字や `code/pre/svg/math/[translate=no]/.notranslate` subtree を事前除外
