...
- **モデル重みは実行時 DL + Cache API**（**重み=データはストア審査上 DL 可**）: `MODEL_URLS`（MI-GAN / PaddleOCR det・rec・dict）を `fetchModelBytes`/`loadDict` が `caches`(`rt-onnx-models-v1`) 経由で取得。セッションは doc 生存中 lazy キャッシュ（失敗時は破棄して次回再試行）
...
- **★verify-on-device（headless 検証不可・実機で要確認）**: ① `MODEL_URLS` が 200 を返すか（404 なら cloud フォールバック）② MI-GAN mask 極性（255=keep/0=hole）③ DET/REC の入出力テンソル名・**チャネル順 RGB/BGR**・正規化 ④ `REC_HEIGHT`(32 か 48 か) ⑤ CTC dict オフセット（`index-1`）⑥ WebGPU→WASM フォールバックと wasm ロード ⑦ huggingface.co の CORS。ズレたら inference.js の該当箇所（コメントに `★` 印）を調整する
