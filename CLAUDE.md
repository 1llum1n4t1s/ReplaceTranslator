...
- **★verify-on-device（headless 検証不可・実機で要確認）**: URL/テンソル名/正規化/`REC_HEIGHT`/dict オフセット/BGR/mask 極性は **実 ONNX バイナリで実測確認済み**（変更不要）。残る実機確認は ① WebGPU→WASM フォールバックと wasm ロードが offscreen で通るか ② huggingface.co / raw.githubusercontent.com の CORS が拡張 fetch で通るか ③ 実画像で OCR 文字列・bbox が妥当か（`det box_thresh` 0.6 が渋ければ 0.5 へ緩める余地）。DevTools の offscreen document コンソールで `MODEL_URLS` の 200 と OCR `original` を確認できる
