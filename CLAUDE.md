...
- **box の文字インクスナップ（ズレ低減・依存ゼロ）**: 平坦背景のとき `snapToInk` が box(+余白)内で背景色と差>~60 の画素=文字インクの外接矩形へ box を引き締め、LLM の緩い/ズレた box を実 glyph 位置に補正（消去のはみ出しと配置ズレを低減）。文字無し/探索域ほぼ全面インクなら null で元 box 維持。textured では背景色不定のため非適用
