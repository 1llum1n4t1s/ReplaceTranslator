...
※ Chrome ウェブストア / Firefox Add-ons での配信リンクは公開後に追記します。

## 🚀 使い方
1. ツールバーのアイコンを開き、**「キー」タブ**で使うサービスの API キーを入力して保存（MyMemory はキー不要）
2. **「翻訳」タブ**でサービス・翻訳先言語を選んで「翻訳」。原文に戻すには「原文に戻す」
3. **ページ右下のフローティングボタン（文 → 訳）** をクリックしても翻訳 / 復元をトグルできます（ドラッグで好きな位置へ移動可）
4. 右クリックメニューからも「このページを翻訳」/「原文に戻す」が使えます
5. 「キー」タブで**画像内テキストの翻訳**を ON にすると、画像にホバーして「訳」で画像内の文字も翻訳できます（実験的・vision 対応 LLM）

### 翻訳元 / 翻訳先
- **翻訳元** — 「自動判定」のままにすると、翻訳先と異なる言語だけが翻訳されます（言語が混ざったページもそのまま使えます）。特定の言語だけ訳したいときは明示指定もできます
- **翻訳先** — 訳したい言語を選びます

## 📊 トークン使用量
ポップアップ下部に、当月の使用トークン数がプロバイダ別・合計で表示されます。月が替わると自動的に 0 から計測し直します（過去の月の分はブラウザ内に保持されます）。

## 🌐 対応言語
日本語・英語・中国語（簡体 / 繁体）・韓国語・スペイン語・フランス語・ドイツ語・ポルトガル語・ロシア語・イタリア語ほか主要言語。

## 🔒 プライバシー
- API キーとトークン使用量は**お使いのブラウザ内にのみ保存**されます
