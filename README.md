...
各社のクラウド LLM またはキー不要の MyMemory を使って、表示中の Web ページを **その場で置換翻訳**する Chrome / Firefox 拡張機能です。Chrome 標準のページ翻訳のように原文を訳文へ差し替え、ワンクリックで原文に戻せます。LLM を選ぶと、文脈をふまえた自然な翻訳が得られます。

## ✨ 特徴
- **インプレース置換翻訳** — ページの原文を訳文に差し替え、トグルで原文に復元
- **各社 LLM 対応** — OpenAI / Anthropic (Claude) / Google Gemini / xAI (Grok) / OpenRouter / DeepSeek / Groq / Sakana AI (Fugu) を切り替え（自分の API キーを使用）
- **モデルを選択** — モデル一覧を更新すると各社の新しいモデルも表示。価格情報がまだないモデルは価格欄に「—」を表示
...
## 🚀 使い方
1. ツールバーのアイコンを開き、**「API設定」タブ**で使うサービスを選んで API キーを入力（欄からフォーカスを外すと自動保存。MyMemory はキー不要）。モデル一覧はキー保存時に取得され、モデル欄の「更新」から再取得できます。モデルを選ぶと、対応モデルではその直下で推論 effort も設定できます
2. **「翻訳」タブ**で翻訳先言語を選んで「翻訳」。原文に戻すには「原文に戻す」
3. 自動翻訳を止めたいサイトは「除外リスト」タブへ URL・ホスト・ワイルドカードを1行ずつ入力します。「現在のサイトを追加」を押すと、表示中サイトのホスト名を追加できます。右クリックメニューから追加・削除することもできます
...
## 👩‍💻 開発者の方へ
開発手順は [CONTRIBUTING.md](CONTRIBUTING.md)、設計は [DESIGN.md](DESIGN.md)、エージェント向け作業規約は [AGENTS.md](AGENTS.md) を参照してください。
