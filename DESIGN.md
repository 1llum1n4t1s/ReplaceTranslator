...
モデル一覧はAPIキーの保存時と利用者が明示更新したときに各社APIから取得し、翻訳できない用途と中身が変わる `latest` エイリアスを除いて新しい順に表示する。日付付きIDは新モデルの唯一の公開名になり得るため一律には除外せず、Anthropicだけは日付を外したエイリアスにまとめる。価格カタログに未収録のモデルも選択可能にし、価格は models.dev のキャッシュと同梱表で補う。不明なら価格欄に「—」を表示する。表示件数を上位10件に絞っても、選択中のモデルの有効性は取得した全件で判定し、一覧外の有効な選択を勝手に置き換えない。

モデルごとの推論制御は対応を確認した系列にだけ送る。未知モデルへ推論量や温度を推測して指定するとAPIが要求を拒否し得るため、そのモデルの既定値に任せる。テキスト翻訳中にモデルの404を受けた場合はlive一覧で代替を探し、同じバッチを1回だけ再試行する。画像では一覧APIがvision入力への対応を保証しないため、別のテキストモデルへ自動切替しない。具体的なフィルター、キャッシュ、切替手順は [references/architecture.md](references/architecture.md) の「動的モデル取得 & バッチ自動学習」を参照する。
